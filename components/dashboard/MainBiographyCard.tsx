...
export function MainBiographyCard({ biography, userName, userId, onDeleteClick, onCreateClick }: MainBiographyCardProps) {
  ...
  const getStatusBadgeConfig = (status: string) => {
    switch (status) {
      ...
      case 'suspended_pending_verification':
        return { text: t.dashboard.statusSuspendedPendingVerification, className: 'bg-brand-wineDark text-brand-paper' };
      case 'revision_requested':
        return { text: t.dashboard.statusRevisionRequested, className: 'bg-brand-mustardLight text-brand-ink' };
      case 'revision_pending_review':
        return { text: t.dashboard.statusRevisionPendingReview, className: 'bg-brand-mustardLight text-brand-ink' };
      case 'revision_overdue':
        return { text: t.dashboard.statusRevisionOverdue, className: 'bg-brand-wineDark text-brand-paper' };
      default:
      ...
      <div className="p-6 sm:p-8 text-center bg-transparent rounded-2xl">
        ...
        {onCreateClick && (
          ...
          >
            ...
            <span>{t.dashboard.startWriting}</span>
          </Button>
        )}
      </div>
    );
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-base text-center text-muted-foreground mb-6">
            {getStatusBadgeConfig(biography.status).text}. {privacyInfo.text}.
          </p>
          {biography.status === 'under_review' && (
            <p className="text-sm text-center mb-4">{t.dashboard.underReviewMessage}</p>
          )}
